...
import {TT2_ACHIEVEMENTS} from './tt2-achievements.ts';
export {TT2_ACHIEVEMENTS,ACHIEVEMENT_PANEL_TEXT} from './tt2-achievements.ts';
...
import { EXTRA_HEROES, ARTIFACTS, MONSTERS, DAILY_TASKS, PERKS, ACTION_TYPES, type Effect } from './content.ts';
export { ARTIFACTS, MONSTERS, DAILY_TASKS, PERKS, ACTION_TYPES } from './content.ts';
...
function damage(s:State,hit:Big){if(compare(hit,{...ZERO})<=0)return;s.hp=subtract(s.hp,hit);if(compare(s.hp,{...ZERO})>0)return;
 ...
 if(boss){if(s.stage>=16&&(s.stage-16)%20===0&&s.stage> s.tt2!.gearMilestone&&dropGear(s.tt2!,s.best)){s.tt2!.gearMilestone=s.stage;s.tt2!.equipmentCollected++;}s.bossKills++;s.stage=Math.min(STAGE_CAP,s.stage+1);s.best=Math.max(s.best,s.stage);s.kills=0;
...
export function apply(s:State,a:Action){advance(s,a.at);const i=a.index??0,t=s.tt2!;
 ...
 if(a.type==='daily'&&t.loginAt!==dayAt(s.last)){
  ...
  if(reward.reward==='Equipment')for(let n=0;n<reward.amount;n++)if(dropGear(t,s.best))t.equipmentCollected++;
