...
import {type Big,type BigLike,ZERO,ONE,isBig,big,fromNumber,toNumber,add,subtract,multiply,power,pow,scale,compare,sum,max as bigMax,atLeastZero,ceil as bigCeil,ratio} from './big-number.ts';
...
function damage(s:State,hit:Big){if(compare(hit,{...ZERO})<=0)return;s.hp=subtract(s.hp,hit);if(compare(s.hp,{...ZERO})>0)return;
 ...
 s.gold=add(s.gold,goldReward(s,boss?'boss':chest?'chest':'monster'));s.totalKills++;s.daily.kills++;
 if(boss){if(s.stage>=16&&(s.stage-16)%20===0&&s.stage> s.tt2!.gearMilestone&&dropGear(s.tt2!,s.best))s.tt2!.gearMilestone=s.stage;s.bossKills++;s.stage=Math.min(STAGE_CAP,s.stage+1);s.best=Math.max(s.best,s.stage);s.kills=0;
...
export function advance(s:State,to:number){hydrate(s);if(!Number.isFinite(to))return s;to=Math.max(s.last,to);const gap=to-s.last;advanceEggs(s.tt2!,to);
 ...
 if(gap>30000){const seconds=Math.min(gap/1000,8*3600);s.tt2!.mana=Math.min(manaMax(s),s.tt2!.mana+baseManaRegen(s)*manaSeconds(s.tt2!,s.last,s.last+seconds*1000));const offline={...s,last:to,active:Array(6).fill(0)};s.gold=add(s.gold,scale(reward(offline),Math.min(ratio(dps(offline),bigMax({...ONE},health({...offline,farming:true}))),2)*seconds*.5));s.last=to;s.active.fill(0);if(isBoss(s)){s.farming=true;s.kills=0;spawn(s);}return s;}
...
export function apply(s:State,a:Action){advance(s,a.at);const i=a.index??0,t=s.tt2!;
 ...
 if(a.type==='tap'&&s.last-s.lastTap>=45){s.lastTap=s.last;s.taps++;s.daily.taps++;t.lastCrit=tt2Random(t)<critChance(s);let n=scale(tapDamage(s),t.lastCrit?10:1);if(s.active[1]>s.last&&tt2Random(t)<SKILL_DATA[1].second[s.skillLevels[1]-1])n=scale(n,skillPower(s,1));t.lastHit=n;damage(s,n);if(chargePet(t)){t.lastPetHit=petAttackDamage(s);t.petAttacks++;damage(s,t.lastPetHit);}}
 ...
 if(a.type==='daily'&&t.loginAt!==dayAt(s.last)){
  ...
  if(reward.reward==='MonsterGold')s.gold=add(s.gold,scale(goldReward(s,'monster'),reward.amount));
