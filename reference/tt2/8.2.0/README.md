...
證據位於 [enhancement-parser-evidence.json](enhancement-parser-evidence.json)。`python tools/audit-enhancement-parser.py` 可用本機 Capstone／pyelftools 重核：逐位元組比對 APK 中的原生程式庫、11 處指令、方法名稱、6 個字串引用及 OverwriteExisting 列舉值。匯入器不模擬全部 .NET 字串解析；遇到不支援的數值、未知 ID 或欄位變更會停止，避免靜默遺漏。`quarantine.json` 保留已解決記錄與尚未匯入的表；其他表仍不允許覆寫重複 ID。

R02 尚餘：剩餘資源盤點中其他未匯入資料表的 schema，以及缺少來源證據的模式／可用性分類；已完成突襲技能、卡片費用、玩家、敵人、部位、大師階級、單人突襲、研究、票券與忠誠表；外觀目錄與其支援表、伺服器變數與已知來源變體已補齊；商店禮包、突襲關卡與區域已匯入，配送與複合獎勵格式已整理；原生解析與選擇索引已核對，發獎及配送仍待驗證。完成後才進入 R03 存檔遷移。本目錄的逐 ID pending 記錄是核對清單，不是玩法完成證據。
...
本次追加伺服器變數與來源變體共 6 張表：ServerVarsInfo 12 筆、ServerVarOverride 25 筆（來源 26 列）、ArtifactCostInfo_A 194 筆、TitanScalingInfo_A／B／C 共 21 筆。新增 [servervars-parser-evidence.json](servervars-parser-evidence.json)，可用 `python tools/audit-servervars-parser.py` 以本機 Capstone／pyelftools 重核：逐位元組比對 APK 中的原生程式庫、28 處指令、14 個方法名稱與 8 個字串引用。

ServerVarOverride 的重複鍵已解除隔離：`LoadServerVarsOverrideFromInfoDoc`（RVA 0x24a8dcc）按遞增列號讀取 ServerVarsKey／Value，先以 IsNullOrWhiteSpace 跳過空白鍵、再跳過空值，最後呼叫 Dictionary.set_Item，故同鍵最後成功解析的列生效。pet_paradise_shovel_purchase_daily_limit 出現在第 14 與第 18 列（列號從 0 起），兩列的值都是 3；26 列整理為 25 個 ID，兩列都保留在 rowResolution.duplicateHistory。這項政策只適用本表，其他表仍拒絕重複鍵。

ServerVarsInfo 由 `LoadServerVarsFromInfoDoc`（RVA 0x24a8f78）載入，鍵為 ServerVarsKey，值欄位在執行期於 iOS／Google／Amazon 三欄中擇一，再由 SetVarsFromAttributes 套用到靜態欄位。參考目錄保留三欄原值，不代為選平台。**這兩張表都是安裝包內附值，不是線上生效的伺服器覆蓋值**，分類為 bundled-server-variable、liveAvailability=unknown。maxStage 98000、relicsStageMax 180000 等數字是資料事實，不能直接當成線上上限改寫遊戲。

泰坦縮放為 A／B 測試來源：`MonsterModel.ParseTitanScalingInfo`（RVA 0x23252f8）呼叫 `InfoDocs.GetABTestInfoDoc("TitanScalingInfo")`（RVA 0x228804c），後者在執行期取得的 A／B 表名非空且包含所求名稱時改用該表，再交給 LoadAsText。指派的表名不在安裝包內，因此線上實際使用哪一份變體未知，四份一律標 ab-test-source-variant、ab-sheet-assignment=unknown。

新增 `variantOfSource` 欄位與 `validation.json` 的 `sourceVariants`，把七組來源變體與其基準表逐 ID 比對：ArtifactCostInfo_A 與 TitanScalingInfo_C 同基準表完全相同；TitanScalingInfo_A 多出關卡 105／110／120／200／500；TitanScalingInfo_B 多出關卡 2 至 5，且關卡 1 的 BonusAmountA 由 1.00E+00 變為 3.00E-01；三組終局 _1 變體只有同名 ID 的欄位值差異。比對結果只是差異記錄，liveSelection 一律 unknown。

ScheduledBonusInfo 未匯入，記在 `quarantine.json` 的 `tables`。原生沒有針對此表的 InfoDoc 逐列解析器，改由 `ScheduledEventsModel.ParseAllBonuses(Dictionary<string, object>)`（RVA 0x244c84c）與 `ParsedScheduledBonusInfo.TryParse(Dictionary<string, object>, string)`（RVA 0x24498c4）從伺服器字典建立；安裝包內的 4 列中只有 1 列有 BonusType，其餘 3 列只有註解欄位。缺少線上排程內容前不補成正式資料表。

本次追加突襲進度 13 張表：RaidMasterTierLevelInfo 190 筆（TierID＋LevelID 複合鍵，TierID 9999 重複出現）、SoloRaidLevelInfo 1,200 筆（WorldID＋LevelID）、SoloRaidFarmingLevelInfo 120 筆、RaidResearchInfo 94 筆、RaidCardLevelRewardInfo 150 筆、RaidTicketBoostInfo 301 筆、RaidLoyaltyInfo 13 筆、RaidCardBoostedSlotInfo 14 筆、RaidEnemyEnchantmentInfo 3 筆、兩張快速完成加成表 9 筆，以及 RaidMasterTierRewardInfo 與其 _1 變體各 5 筆。引用檢查由 19,202 增為 30,124，errors 與 unresolved 皆為 0。

新增 [infodoc-parser-evidence.json](infodoc-parser-evidence.json)，可用 `python tools/audit-infodoc-parser.py` 重核 10 處指令：`InfoDocs.ParseInfoDoc`（RVA 0x2287a10）按遞增索引把每個標題名寫進 Dictionary<string,int>，使用 set_Item，因此**重複的欄位名以最後一欄的索引生效**，較早的同名欄位在原生無法以名稱取得。這是所有工作表共通的行為，不是單一表例外。

RaidTicketBoostInfo 的 Amount5 與 RaidResearchInfo 的 Total Levels 各重複一次。匯入器只對這兩張已列名的表放行，並在 `columnResolution` 記錄每個重複欄位的來源索引、生效索引，以及被遮蔽欄位的逐列原值，因此沒有任何來源儲存格被丟棄（例如 RaidResearchInfo 第一列的 Total Levels 生效值為 4100，被遮蔽的第 11 欄值為 10）。其他表出現重複欄位仍直接停止匯入。

獎勵解析新增兩個實際出現的純量代號 FortuneRaidCard（RewardID 77）與 PlayerRaidXP（43），並接受 Reward、NewPlayerReward 兩個欄位名。SoloRaidLevelInfo 的 AvatarReward 是 AvatarID 參照而非獎勵字串，八個值都能在 AvatarInfo 解析。解析器仍只支援已觀察到的格式，未知代號或非整數數量一律失敗。
