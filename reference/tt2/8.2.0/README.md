...
`manifest.json` 記錄安裝包、33 張已解析表（8,907 個有效 ID）的 SHA-256，以及 372 個 TextAsset 的大小與雜湊。未解析的資源只列索引，不宣稱它們皆為 CSV 或已完成 schema。
...
本次檢查 15,341 處引用；其中 12 處不在 BonusInfo，但在原生 BonusType 列舉中存在，記錄在 `nativeOnly`。`native-bonus-types.json` 僅保存列舉 ID／數字與來源雜湊，不能用來推定效果公式。沒有未解析的引用 ID，不代表所有公式或所有資源都已驗證。
...
R02 尚餘：其餘支援表 schema，以及缺少來源證據的模式／可用性分類；已完成突襲技能、卡片費用、玩家、敵人及部位表；下一批為商店禮包與突襲關卡配置。完成後才進入 R03 存檔遷移。本目錄的逐 ID pending 記錄是核對清單，不是玩法完成證據。
...
玩家資料的 PlayerRaidProgressionBundle 另列 deferredReferences，因商店禮包表尚未匯入，不能宣稱所有引用已完成；unresolved=0 只適用於目前已涵蓋的引用範圍。None 部位及保護／相對／連動關係依原表保留，不把 66 筆資料等同於 66 個可攻擊部位。
