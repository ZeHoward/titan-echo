...
- 頭目逾時進入金幣農場，可手動重試。
- 10 位英雄自動攻擊，每 25 級傷害加倍，可批量升級 1／10／25 級。
- 6 個技能：影分身、致命一擊、戰爭怒吼、狂戰之怒、黃金之手、天堂之擊。
- 50 關開啟轉生；3 種神器提供永久傷害／金幣加成。
- 5 個環境色調；每 60 秒有仙女金幣。離線收益上限 8 小時，不離線推關。
- 自動存檔每 3 秒一次；最多保留 700 關、英雄／劍士最高 1000 級。
...
已驗證：11 項遊戲規則測試和 1 項整合測試（含帳號隔離、同版本同時寫入、拒絕非法操作、暱稱隔離、排行榜與重啟保存），以及 TypeScript 與兩種正式建置。未做真人長時間平衡測試、大量同時在線壓力測試或瀏覽器互動自動化。
...
本作縮短轉生門檻並重新設計成長數值，未實作原作全數英雄、怪物、神器、裝備、付費商店或錦標賽。伺服器驗證操作與時間、限制點擊頻率，但不宣稱能阻止所有自動點擊工具。
