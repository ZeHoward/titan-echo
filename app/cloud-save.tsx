...
const messages:Record<string,string>={CONFLICT:'另一台裝置已有新進度。已暫停上傳，請先預覽雲端存檔。',NOT_FOUND:'找不到此恢復碼的存檔。',INVALID_KEY:'恢復碼應為 64 位英數字。',NOT_READY:'管理員尚未執行 Apps Script setup。',SCHEMA_MISMATCH:'存檔分頁欄位不符，請聯絡管理員。',BUSY:'雲端忙碌，稍後重試。',CAPACITY:'雲端帳號數已達上限。'};
export default function CloudSave({open,basePath,onClose}:{open:boolean;basePath:string;onClose:()=>void}) {
 ...
 function error(e:unknown){const text=e instanceof Error?e.message:'連線失敗';setStatus(messages[text]||text);if(text==='CONFLICT')enable(false);}
 ...
 useEffect(()=>{alive.current=true;void run(async()=>{
  ...
  if(!validEndpoint(url)){setStatus('等待管理員完成 Google 雲端設定；本機存檔照常運作。');return;}
 ...
 function downloadKey(){const blob=new Blob(['Titan Echo 恢復碼（等同密碼，請勿分享）\n'+key+'\nhttps://zehoward.github.io/titan-echo/\n'],{type:'text/plain;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='Titan-Echo-Recovery.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
 async function exportBackup(){await run(async()=>{const backup=await readBrowserSave('before-cloud-restore');const blob=new Blob([JSON.stringify(backup,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='Titan-Echo-Before-Cloud-Restore.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});}
 if(!open)return null;
 return <div className="modal-backdrop"><section className="modal" role="dialog" aria-modal="true" aria-label="Google 雲端存檔"><button className="close" onClick={onClose} disabled={pending} aria-label="關閉">✕</button><h2>Google 雲端存檔</h2><p role="status">{status}</p><p>本機每 3 秒保存；連線後每 60 秒備份到雲端。切換裝置前請先儲存，再於另一台載入。同一角色請一次使用一台裝置。</p>
 ...
 {active&&<button className="primary-button" disabled={pending} onClick={()=>void upload()}>立即同步雲端</button>}<button className="setting-row" disabled={pending} onClick={()=>{enable(false);binding.current=null;localStorage.removeItem(STORAGE);setPreview(null);setStatus('已停止自動同步，本機與雲端存檔均保留。');}}>停止自動同步</button></>:<p>管理員完成 Apps Script 部署後即可啟用，不影響目前的遊玩進度。</p>}
