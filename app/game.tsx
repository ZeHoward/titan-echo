...
import { SKILLS, SKILL_ORDER, skillMana, manaMax, fresh, advance, apply, hydrate, health, isBoss, tapDamage, dps, relicGain, fmt, critMultiplier, monsterIndex, monsterCount, bossDuration, buildDamage, fairiesUnlocked, qteReady, petBurstTaps, type State, type Action } from '../lib/engine';
import { QTE_TYPE } from '../lib/tt2-rules';
...
export default function Game({pagesMode=false,basePath='/'}:{pagesMode?:boolean;basePath?:string}){
 ...
 const theme=themeIndex(s.stage),region=theme%10;const boss=isBoss(s);const monster=MONSTERS[monsterIndex(s)];const seconds=Math.max(0,Math.ceil(((s.trial?.endAt??s.bossEnd)-s.last)/1000));
 // 妖精現在走 QTE 排程：冷卻結束才出現，沒點的話 expireTime 到了就飛走並重排冷卻。
 const fairy=fairiesUnlocked(s)&&qteReady(s.tt2!,QTE_TYPE.Fairy);const fairyLeft=fairy?Math.max(0,Math.ceil((s.tt2!.qteExpireAt[QTE_TYPE.Fairy]-s.last)/1000)):0;
 // 雷霆爆發：ready 的時候戰鬥區的點擊會累積連打進度，寵物那一欄改成顯示它。
 const burstReady=combatPet>=0&&qteReady(s.tt2!,QTE_TYPE.PetAttack);const burstTaps=burstReady?petBurstTaps(s):0;
