...
export default function TT2Panels({s,tab,ready,basePath,act,onPrestige}:Props){
 ...
 {tab==='heroes'?<>
  ...
  <div className="sword-master"><div className="hero-top"><span className="avatar master">⚔️</span><div><h3>劍術大師</h3><p>等級 {s.level} · 點擊 {fmt(tapDamage(s))}</p><p>基礎劍傷 {fmt(swordMasterBaseDamage(s))}</p><p>里程碑累計 ×{fmt(playerMilestone(s.level))}</p><p>{nextMilestone?`下一里程碑：等級 ${nextMilestone.level} · 累計 ×${fmt(fromText(nextMilestone.total))}`:'已達本版等級上限'}</p></div><button className="buy-button" disabled={s.level>=2000||s.gold<price(-1)} onClick={()=>act({type:'upgrade',amount:qty})}>升級<small>●{priceText(-1)}{qty===0?' 起':''}</small></button></div></div>
  ...
  <div className="content-heading"><h3>技能樹</h3><p>可用 {t.points} 點 · 已分配 {spentPoints(t)} 點 · 第 100 關起每 50 關 1 點</p></div>
  <div className="collection-tabs">{branches.map((b,i)=><button key={b} className={branch===b?'chosen':''} onClick={()=>setBranch(b)}>{branchNames[i]}</button>)}</div>
  <p className="panel-note">費用、前置條件與等級效果來自 7.5 資料。傷害、金幣與魔力加成已連接；跳關、連擊操作、公會、寵物等特殊行為仍待完成，勿將資料表視為所有行為已還原。</p>
  {TT2_TREE.map((k,i)=>({k,i})).filter(({k})=>k.branch===branch).map(({k,i})=><article className="feature-card" key={k.id}><h3>{TALENT_NAMES[k.name]}<small>等級 {t.tree[i]} / {k.max}</small></h3>{k.effects.map(e=><p key={e.type}>{effectLabel(e.type)}：{t.tree[i]?fmt(e.values[t.tree[i]]):'未啟用'}</p>)}<small>本樹累計 {k.required} 點 · 前置 {k.prerequisite==='None'?'無':TALENT_NAMES[TT2_TREE.find(n=>n.id===k.prerequisite)?.name||'']}</small><button className="buy-button" disabled={!canBuyTalent(t,i,s.best)} onClick={()=>act({type:'talent',index:i})}>{t.tree[i]>=k.max?'已滿級':`升級 · ${k.cost[t.tree[i]]} 點`}</button></article>)}
  ...
  {RESOURCE_PERKS.map((p,i)=>{const level=perkLevel(t,i,s.last),full=level>=perkLimit(t),ends=t.perkEnds[i].filter(end=>end>s.last).sort((a,b)=>a-b);return <article className="feature-card" key={p.id}><h3>{p.icon} {p.name}<small>{level} / {perkLimit(t)} 層</small></h3><p>{p.description}</p>{level>0&&<p>{i===0?`目前回復倍率 ×${perkValue(t,i,s.last)}`:`每 ${Number(perkValue(t,i,s.last).toFixed(2))} 秒自動升級`}</p>}{ends.map((end,j)=><p key={j}>第 {j+1} 層剩餘 {Math.ceil((end-s.last)/60000)} 分鐘</p>)}{full&&<p>層數已滿；再使用會把剩餘時間最短的那層換成完整十二小時。</p>}<div className="feature-actions"><button className="buy-button" disabled={s.diamonds<p.cost} onClick={()=>act({type:'resourcePerk',index:i})}>使用 · 💎{p.cost}</button><button className="outline-button" disabled={t.perkTokens<1} onClick={()=>act({type:'resourcePerk',index:i,amount:1})}>使用登入兌換次數</button></div></article>;})}
  <p className="panel-note">每層各計時十二小時，蛻變不重置；層數滿了仍可使用，會把剩餘時間最短的那層換成完整十二小時。登入獎勵暫可自選這兩種增益；原版的隨機配發在第 1200 關解鎖、另有票券制，黃金雨的立即金幣也尚未實作，離線自動升級仍待核實。</p>
