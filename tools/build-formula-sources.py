...
FORMULAS = [
    entry(id='monsterHealth', module='lib/engine.ts', export='health',
          ...
          parts=[
              ...
              entry(part='基礎值 18 與每關成長率 1.32', status='baseline-75',
                    note='沿用 7.5 基準。安裝包的 MonsterHPScaling 只給倍率修正而非基礎曲線，且 A／B 變體不同'
                         '（B 在關卡 1–3 為 0.3／0.4／0.475），屬 A／B 指派結果，無法從安裝包單獨確定。'),
    ...
    entry(id='monsterGold', module='lib/engine.ts', export='goldReward',
          ...
          parts=[
              entry(part='基礎值 5 與成長率 1.27', status='baseline-75',
                    note='沿用 7.5 基準，安裝包未找到對應的逐關金幣曲線表。'),
