...
FORMULAS = [
    ...
    entry(id='mana', module='lib/engine.ts', export='manaMax',
          ...
          parts=[
              entry(part='基礎上限 200', status='table-differs', ref='servervar-defaults.json',
                    note='原生沒有固定上限：PlayerModel.RefreshManaCap 呼叫 '
                         ...
                         'manaCapPerSkill 35，所以六個技能全解鎖是 210，早期則遠低於引擎固定的 200。'
                         '**尚未採用**：改了會讓早期魔力上限只有 35–70，'
                         '而魔力回復那一半的算式還沒讀完，兩者是配套的。'),
              entry(part='基礎回復 2', status='table-differs', ref='servervar-defaults.json',
                    note='原生 PlayerModel.RefreshManaRegen 以 [ServerVar] manaRegenBaseInMinutes '
                         '（編譯期預設值 2）為底，加上 Bonus(ManaRegen)、乘上 Bonus(ManaRegenMult)，'
                         '最後還有一次除法與一次乘法把它換算成每秒回復量——**那兩步尚未讀完**，'
                         '所以不確定 2 的單位是「每分鐘回復點數」還是「回滿所需分鐘數」。'
                         '引擎目前照 7.5 基準當成每分鐘 2 點。')]),
