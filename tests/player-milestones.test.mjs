...
test('milestone changes actual tap and pet damage while ship gets no tap bonus',()=>{
 ...
 s.level=80;
 near(buildDamage(s,'tap')/buildDamage(baseline,'tap'),intrinsic(80)*32);
 near(petAttackDamage(s)/petAttackDamage(baseline),intrinsic(80)*32);
 near(buildDamage(s,'ship')/buildDamage(baseline,'ship'),1);
 s.skillLevels[0]=1;baseline.skillLevels[0]=1;
 near(buildDamage(s,'clone')/buildDamage(baseline,'clone'),(intrinsic(80)*32)**.6);
});
...
test('bulk upgrade crosses milestones once and loading never compounds the multiplier',()=>{
 ...
 const damage=buildDamage(s,'tap');near(damage,intrinsic(105)*128);
