...
test('equipment formula uses both nonlinear terms, equipment changes actual damage',()=>{
 ...
 const before=N(tapDamage(s));s=apply(s,{type:'equip',index:1,at:1000});assert.ok(N(tapDamage(s))>before);
 const gold=N(goldReward(s,'monster'));s.tt2.equipped[0]=-1;assert.equal(N(goldReward(s,'monster')),gold);
});
...
test('daily rewards cannot be claimed twice and follow the fourteen-day table',()=>{
 let s=fresh(1000);s=apply(s,{type:'daily',at:1000});assert.equal(s.tt2.loginIndex,1);const gold=N(s.gold);
 s=apply(s,{type:'daily',at:1000});assert.equal(N(s.gold),gold);assert.equal(s.tt2.loginIndex,1);
