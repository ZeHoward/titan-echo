...
test('combat is independent of UI tick versus save replay partitions',()=>{
 const direct=fresh(1000);direct.heroes[0]=20;direct.hp=B(1);
 ...
 for(const key of ['stage','kills','totalKills','gold','hp'])assert.equal(N(frames[key]),N(direct[key]),key);
...
test('snapshot save preserves exact displayed battle and isolates later taps',async()=>{
 const initial=await (await browserRequest('/api/game')).json();const live=structuredClone(initial.state);live.gold=B(100);live.hp=B(7);live.kills=4;
 const expected=structuredClone(live),pending=saveBrowserSnapshot(live,initial.revision,'snapshot-a');
 live.hp=B(2);live.kills=5;live.gold=B(200);
 const saved=await pending;assert.deepEqual(saved.state,expected);assert.equal(N(live.hp),2);
 ...
 assert.equal(N((await readBrowserSave()).state.gold),200);
});
