...
test('endgame source variants preserve matching IDs with different activation and values', () => {
  ...
  const endgameRanks = r => r.field === 'RankReward' && r.table.startsWith('EndgameSeasonRewardInfo');
  assert.equal(report.deferredReferences.filter(endgameRanks).length, 0);
  assert.equal(report.rewardReferences.filter(endgameRanks).length, 8);
});
