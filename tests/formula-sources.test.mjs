...
test('the counts are recorded, so a source changing status is a visible change', () => {
  assert.deepEqual(report.counts, {
    'table-differs': 10, invented: 10, table: 18, default: 15, server: 4, native: 10,
  });
  ...
  assert.equal(unverified, 14);
  assert.equal(report.counts['table-differs'], 10, '已知但未照做的項目要看得見');
});
...
test('每一處與安裝包不一致的地方都寫下來了', () => {
  ...
  assert.equal(differs.length, 10);
...
test('the readable copy matches the register it was rendered from', () => {
  assert.match(readable, /共 31 條公式、67 項來源條目/);
