...
test('every core formula names a source, and every source it names exists', () => {
  ...
  assert.equal(report.formulas, 44);
  assert.equal(report.parts, 140);
});
...
test('the counts are recorded, so a source changing status is a visible change', () => {
  assert.deepEqual(report.counts, {
    'table-differs': 17, invented: 25, table: 20, default: 23, server: 4, native: 51,
  });
  ...
  assert.equal(report.counts['table-differs'], 17, '已知但未照做的項目要看得見');
});
...
test('每一處與安裝包不一致的地方都寫下來了', () => {
  ...
  assert.equal(differs.length, 17);
...
test('the readable copy matches the register it was rendered from', () => {
  assert.match(readable, /共 44 條公式、140 項來源條目/);
