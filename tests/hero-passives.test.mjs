...
test('Maya passives unlock at CSV boundaries and reach actual damage, gold and mana',()=>{
 const s=fresh(1000),tap=tapDamage(s),gold=goldReward(s,'chest');
 s.heroes[0]=19;assert.equal(stateEffect(s,'CritDamage'),1);
 s.heroes[0]=20;assert.equal(stateEffect(s,'CritDamage'),1.1);assert.equal(tapDamage(s),tap*1.1);
 s.heroes[0]=59;assert.equal(critChance(s),.02);
 s.heroes[0]=60;assert.equal(critChance(s),.021);
 s.heroes[0]=99;assert.equal(goldReward(s,'chest'),gold);
 s.heroes[0]=100;assert.ok(Math.abs(goldReward(s,'chest')/gold-1.1)<1e-12);
...
test('hero milestones use the cumulative row once at the level boundary',()=>{
 const s=fresh(1000);s.heroes[1]=9;const before=heroDps(s,1);
 s.heroes[1]=10;assert.ok(Math.abs(heroDps(s,1)/before-(10/9)*1.035*3)<1e-10);
 s.heroes[1]=30;
 assert.equal(heroDps(s,1),131*30*1.035**29*9);
});
