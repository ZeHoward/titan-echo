...
| 判定 | 意義 | 個數 |
|---|---|---|
| `live` | 專案有來源、引擎有讀：正常運作。 | 89 |
| `dead-native-uses-it` | 專案有來源、引擎沒讀，而且原生有取值點：**這些是可以接上的**。 | 139 |
...
- `CritBoostSkillCritDamage` ← TT2_TREE
- `DamagePerManaCap` ← TT2_GEAR、TT2_SETS
- `DamagePerSkillPoint` ← TT2_GEAR、TT2_SETS
- `GoldPerSwordMasterLevel` ← TT2_SETS
...
- `MonsterCountPerStage` ← TT2_SETS

### ActiveSkillModel（2）

- `AllActiveSkillCap` ← TT2_SETS
...
- `ActivePetLevel` ← TT2_SETS、TT2_TREE
...
### BonusModel / InactiveGameplayModel / MonsterController（1）

- `MultiMonsters` ← TT2_ARTIFACTS、TT2_SETS、TT2_TREE

### BonusModel / InactiveGameplayModel / MonsterController / PlayerModel（1）

- `MultiMonstersMaxCount` ← TT2_TREE

### BossTimerScript（1）

- `BossTimer` ← TT2_TREE

### ChestersonTitanScript（1）
...
- `FairySpawnChance` ← TT2_ARTIFACTS、TT2_SETS、TT2_TREE
